...
expected outcomes and other moments in the outcome distribution (e.g., variance, skewness, etc.).
...
Cell values within this matrix can be set to one if we want to "count" that health state - age combination in our outcome measure, and zero otherwise.[^reward]
...
For remaining life expectancy, we define $\mathbf{H}$ such that each cell receives a value of one. 

Occupancy-based outcomes with partial rewards (e.g., YLDs, QALYs) require an additional matrix $\mathbf{V}$. 
