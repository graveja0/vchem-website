...
if (!file.exists(here(glue("blog/posts/modeling-dalys/_data/gbd_population_{location}-{life_table_year}.rds")))) {
  ...
  df_population %>% write_rds(here(glue("blog/posts/modeling-dalys/_data/gbd_population_{location}-{life_table_year}.rds")))
} 

df_population <- read_rds(here(glue("blog/posts/modeling-dalys/_data/gbd_population_{location}-{life_table_year}.rds")))
```
...
#| code-fold: true
#| code-summary: GBD Cause of Death Data Table
...
#| code-fold: true
#| code-summary: GBD Incidence Rate (per 100k population) Data Table
...
Our next step is to parameterize the model.
...
result1 <- cbind(LE, YLD, YLL, DALY, QALY_DALY) %>%
  as.data.frame() %>%
  ...
  dplyr::select(approach, strategy, LE_LT, LE, YLD, YLL, DALY, QALY_DALY) 

result1 %>% 
    kable(digits = 3, col.names = c("Approach","Scenario","Life Expectancy (Life Table)","Life Expectancy (Model)","YLDs","YLLs","DALYs","QALY-like DALY")) %>% 
    kable_styling()
```
...
QALY_DALYt2 <- trace1 %>% map( ~ ({
    tmp = as.matrix(.x) %*% qaly_daly_
    tmp 
}))

QALY_DALY2 = QALY_DALYt2 %>% map(~sum(.x*  disc * gen_wcc(params1$omega, method = params1$cycle_correction)))


result2 <- cbind(LE2, YLD2,YLL2,DALY2,QALY_DALY2) %>%
    as.data.frame() %>%
    mutate_all(~as.numeric(.))  %>%
    rownames_to_column(var="strategy") %>%
    mutate(approach = "Transition States") %>%
    dplyr::select(approach, strategy, LE = LE2, YLD=YLD2, YLL=YLL2, DALY=DALY2,QALY_DALY=QALY_DALY2)
...
tib %>%
    arrange(strategy) %>%
    select(-strategy) %>%
    kable(digits = 3, col.names = c("Approach","Life Expectancy (Life Table)","Life Expectancy (Model)","YLDs","YLLs","DALYs","DALY-like QALY")) %>% 
    kable_styling() %>% 
...
qaly_daly_ <- rep(1,length(params3$ages))
QALY_DALY3_YLL_ <- params3 %>% healthy_longevity_occupancy(H = H, V = V_LE)
QALY_DALY3_ <- map2(QALY_DALY3_YLL_,YLD3_,~(.x+.y))
...
QALY_DALY3 <- QALY_DALY3_ %>% map(~({
  tmp <- (kronecker(t(c(1,rep(0,params3$omega-1))) ,diag(params3$tau)) %*% as.matrix(.x))
  tmp[1,1]
}))


QALY_DALY3 = QALY3 %>% map(~(NA))

result3 <- cbind(LE3, YLD3,YLL3,DALY3,QALY_DALY3) %>%
    as.data.frame() %>%
    mutate_all(~as.numeric(.))  %>%
    rownames_to_column(var="strategy") %>%
    mutate(approach = "Markov Chain With Rewards") %>%
    dplyr::select(approach, strategy, LE = LE3, YLD=YLD3, YLL=YLL3, DALY=DALY3,QALY_DALY=QALY_DALY3)
...
```{r table-diff-qaly-daly}
#| code-fold: true
#| code-summary: DALY changes, contd
#| tbl-cap: Disability-Adjusted Life Year Outcome Changes Relative to Natural History Model
...
tib %>% 
    ...
    unite(outcome,outcome,strategy) %>% 
    spread(outcome,value) %>% 
    select(approach,starts_with("QALY_DALY")) %>% 
    mutate(QALY_DALY_prevent = QALY_DALY_prevent - QALY_DALY_natural_history,
           QALY_DALY_treat = QALY_DALY_treat - QALY_DALY_natural_history,
           QALY_DALY_prevent_treat = QALY_DALY_prevent_treat - QALY_DALY_natural_history) %>% 
    select(approach, QALY_DALY_natural_history, QALY_DALY_prevent, QALY_DALY_treat, QALY_DALY_prevent_treat) %>% 
