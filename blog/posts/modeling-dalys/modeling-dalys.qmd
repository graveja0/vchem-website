---
title: "Structuring Models for Multidimensional Health Outcomes"
subtitle: "Construction of a Decision-Analytic Discrete Time Markov Model Using Global Burden of Disease Data"
author: John Graves
date: "2023-12-15"
categories: [CEA]
editor_options: 
...
params <- list(
    ...
    horizon = 110,    # model time horizon (in years)  
...
As an alternative option that provides age-specific (i.e., ungrouped) rates, we also include a parametric morality model (based on the [Heligman-Pollard model, which captures mortality across ages quite well](https://www.soa.org/globalassets/assets/library/research/transactions-of-society-of-actuaries/1990-95/1992/january/tsa92v446.pdf)).[^explore]

[^explore]: To explore more on parametric mortality modeling, see our interactive tutorial [here](https://graves-vanderbilt.shinyapps.io/1-mortality-modeling/#section-construct-a-simple-alive-dead-model). 
...
With the basic parameters and parameter functions defined, we can now move to our first approach. Under this approach, we will separate cause-specific and non-CVD deaths as separate health states. The updated state transition diagram is shown in @fig-diagram1:

![State Transition Diagram for Approach 1](media/healthy-cvd-dead-deadcvd.png){#fig-diagram1}
...
We next draw upon the transition probabilty matrices to create a Markov trace (@tbl-markovtrace1):
...
Our next checkpoint is designed to assess whether CVD prevalance at any age matches the prevalance value in the GBD data. @fig-prevalence below shows this using the parameters above. 
...
Our results can be summarised in a step-by-step protocol for calculations of healthy longevity.

1. Start with the transition matrices $\mathbf{U}_{x}(x=1, \ldots, \omega)$, the age advancement matrices $\mathbf{D}_{j}(j=1, \ldots, s)$, and the mortality matrices $\mathbf{M}_{i}(i=1, \ldots, \omega)$.
2. Construct the block-structured age $\times$ stage-classified matrices $\tilde{\mathbf{U}}, \tilde{\mathbf{M}}$, and $\tilde{\mathbf{P}}$ following equations (9), (10), and (11).
3. To calculate healthy longevity ...

... as lifetime occupancy of health stages, go to 4.

... as lifetime number of transitions among health stages, go to 5.

... as the value of lifetime occupancy of health stages, go to 6 .

... as the value of lifetime transitions among health stages, go to 7 .

4. Healthy longevity as occupancy:

(a) Create the array $\mathbf{H}$ in Figure 2. Insert 1 in the ages and health stages of interest.

(b) Create the reward vector $\mathbf{h}$ by applying the vec operator to $\mathbf{H}$, as in (27).

(c) Create the matrices $\tilde{\mathbf{B}}_{1}$ and $\tilde{\mathbf{C}}_{1}$ following (30) and (31).

(d) Create the matrix of occupancy rewards $\tilde{\mathbf{R}}_{1}$ from $\tilde{\mathbf{B}}_{1}$ and $\tilde{\mathbf{C}}_{1}$ using equation (12).

(e) Create the higher moment reward matrices $\tilde{\mathbf{R}}_{2}, \tilde{\mathbf{R}}_{3}, \ldots$ using (34).

(f) Go to 8 .

5. Healthy longevity as the number of transitions:

(a) For each age $x$ of interest, specify the set $\mathcal{T}_{x}$ of transitions to count.

(b) Construct the indicator matrices $\mathbf{B}_{1}^{(x)}$ and $\mathbf{C}_{1}^{(x)}$ using (38) and (39).

(c) Construct the block-structured matrices $\tilde{\mathbf{B}}_{1}$ and $\tilde{\mathbf{C}}_{1}$ using (41) and (43).

(d) Assemble the matrix $\tilde{\mathbf{R}}_{1}$ as in (12).

(e) Create the higher moment reward matrices $\tilde{\mathbf{R}}_{2}, \tilde{\mathbf{R}}_{3}, \ldots$ using (34).

(f) Go to 8 .

6. Healthy longevity as the value of occupancy:

(a) Assign values to occupancy of each age-stage combination in matrices $\mathbf{V}_{m}$, for each moment $m$ of interest, according to (44).

(b) Create the moment vectors $\mathbf{v}_{m}$ by applying the vec operator to $\mathbf{V}_{m}$, as in (45).

(c) Create the block-structured matrices $\tilde{\mathbf{B}}_{m}$ and $\tilde{\mathbf{C}}_{m}$ as in (47) and (48).

(d) Create the block-structured reward matrices $\tilde{\mathbf{R}}_{m}$ as in (12).

(e) Go to 8 .

7. Healthy longevity as the value of transitions:

(a) For each age $x$, obtain the moments of the value of each transition in $\mathcal{T}_{x}$.

(b) For each age $x$ and each moment $m$, create the matrices $\mathbf{B}_{m}^{(x)}$ and $\mathbf{C}_{m}^{(x)}$ according to (49) and (50).

(c) Construct the block-structured matrices $\tilde{\mathbf{B}}_{m}$ and $\tilde{\mathbf{C}}_{m}$ using (41) and (43) and assemble the matrices $\tilde{\mathbf{R}}_{m}$ as in (12).
(d) Go to 8 .

8. Compute the moments of healthy longevity from the transient matrix $\tilde{\mathbf{U}}$, the transition matrix $\tilde{\mathbf{P}}$, and the reward matrices $\tilde{\mathbf{R}}_{m}$, following equations (13) - (15).


```{r parameters3}
#| code-fold: true
#| code-summary: Parameterize the Healthy Longevity Model (continued)
#| cache: true
...
# not sure why this is needed, but otherwise the length ogets set too long...
...
result4 <- cbind(DALY3_endogenous) %>%
    as.data.frame() %>%
    mutate_all(~as.numeric(.))  %>%
    rownames_to_column(var="strategy") %>%
    mutate(approach = "Markov Chain With Rewards - Endogenous") %>%
    dplyr::select(approach, strategy, DALY=DALY3_endogenous) %>% 
  bind_rows(
     cbind(QALY_DALY) %>%
      as.data.frame() %>%
      mutate_all(~as.numeric(.))  %>%
      ...
      dplyr::select(approach, strategy, DALY=QALY_DALY)
  )
...
#| tbl-cap: Disability-Adjusted Life Year Outcome Changes Relative to Natural History Model
#| eval: false
#| 
tib %>% 
    ...
    mutate(approach = factor(approach,levels = c("Markov Trace","Transition States","Markov Chain With Rewards"))) %>% 
    arrange(approach) %>% 
...
```{r table-diff-qaly}
#| code-fold: true
#| code-summary: QALY plot
#| tbl-cap: Quality Adjusted Life Year Outcome Changes Relative to Natural History Model
#| eval: false
#| 
