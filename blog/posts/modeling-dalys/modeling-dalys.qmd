...
We adapt the protocol in @caswell2021 to calculate, YLL, YLD, and DALY
outcomes using Markov Chain with Rewards approach:
...
matrix.[^vecperm] See @henderson1981 and Appendix B in @caswell2021 for further
information.

[^vecperm]: A function to construct a vec-permultation matrix is provided within the code snippet below. 
...
above, is that we do not calculate outcomes off a trace. Rather, the method utilizes matrix calculus to solve for
expected outcomes and other moments in the outcome distribution (e.g., variance, skewness, etc.).[^expected]

[^expected]: For our purposes here we will focus on expected outcomes---though note that formulas for higher-order moments are provided in @caswell2021 and @caswell2018. 
...
To facilitate understanding, we briefly
classify each of our outcomes (LE, YLL, YLD, DALYs, QALYs) into broad classes corresponding to whether the payoff or "reward" applies to occupancy, or to transitions, within or to a given health state:
...
| Quality-Adjusted Life Years               | Occupancy (utility weights applied to alive health states)              |

: Classification of Health Outcomes

To calculate occupancy-based outcomes, we start with a reward matrix $\mathbf{H}$ (@fig-H-le) 

![Reward matrix $\mathbf{H}$](media/H-LE.png){#fig-H-le fig-align="center" width=50%}


Cell values within this matrix can be set to one if we want to "reward" that health state-age combination in our outcome measure, and zero otherwise.[^reward]

[^reward]: This structure allows us, for example, to estimate outcomes for certain age ranges or other decision-relevant combinations of health state and age. 

For remaining life expectancy, we define $\mathbf{H}$ such that each cell receives a value of one. We use this matrix to define the reward vector $\mathbf{h}$:
...
$$
For YLD outcomes, $\mathbf{V}$ is defined as shown in @fig-V-yld, while for QALY outcomes $\mathbf{V}$ is defined as shown in @fig-V-qaly .

![Reward matrix for YLD Outcome](media/H-YLD.png){#fig-V-yld fig-align="center" width=70%}

![Reward matrix \mathbf{V} for QALY Outcome](media/H-QALY.png){#fig-V-qaly fig-align="center" width=60%}
...
Like many health economic applications, the Healthy Longevity approach makes assumptions partial occupancy in a health state.[^cyclecorr] Specifically, the approach assumes that partial occupancy in a health state receives half the reward---essentially, we draw on an assumption that mid-cycle transitions occur half-way through. 

We combine this assumption, along with the vectorized reward matrices $\mathbf{h}$ and $\mathbf{v}$ to obtain:
...
[^cyclecorr]: For example, half-cycle corrections are often used---though there are other methdos (e.g., Simpson's rule) that are also drawn upon.
...
Alternatively, we may wish to calculate outcomes seprately under different starting ages, and for a specified starting health state (e.g., healthy). This is given by
...
where $\otimes$ is the Kroenecker operator.
...
-   Create the array $\mathbf{H}$.
-   Create the reward vector $\mathbf{h}$ by applying the vec operator
    to $\mathbf{H}$, i.e.,
    $\mathbf{h} = \text{vec } \mathbf{H} \quad \tau \omega \times 1$.
-   Create the matrices $\tilde{\mathbf{B}}_{1}$ and
    $\tilde{\mathbf{C}}_{1}$
-   Create the matrix of occupancy rewards $\tilde{\mathbf{R}}_{1}$ from
    $\tilde{\mathbf{B}}_{1}$ and $\tilde{\mathbf{C}}_{1}$ using equation
    (12).
-   Create the higher moment reward matrices
    $\tilde{\mathbf{R}}_{2}, \tilde{\mathbf{R}}_{3}, \ldots$ using (34).
-   Go to 8 .
...
(a) For each age $x$ of interest, specify the set $\mathcal{T}_{x}$ of
    transitions to count.

(b) Construct the indicator matrices $\mathbf{B}_{1}^{(x)}$ and
    $\mathbf{C}_{1}^{(x)}$ using (38) and (39).

(c) Construct the block-structured matrices $\tilde{\mathbf{B}}_{1}$ and
    $\tilde{\mathbf{C}}_{1}$ using (41) and (43).

(d) Assemble the matrix $\tilde{\mathbf{R}}_{1}$ as in (12).

(e) Create the higher moment reward matrices
    $\tilde{\mathbf{R}}_{2}, \tilde{\mathbf{R}}_{3}, \ldots$ using (34).
...
(a) Assign values to occupancy of each age-stage combination in matrices
    $\mathbf{V}_{m}$, for each moment $m$ of interest, according to
    (44).

(b) Create the moment vectors $\mathbf{v}_{m}$ by applying the vec
    operator to $\mathbf{V}_{m}$, as in (45).

(c) Create the block-structured matrices $\tilde{\mathbf{B}}_{m}$ and
    $\tilde{\mathbf{C}}_{m}$ as in (47) and (48).

(d) Create the block-structured reward matrices $\tilde{\mathbf{R}}_{m}$
    as in (12).
...
(a) For each age $x$, obtain the moments of the value of each transition
    in $\mathcal{T}_{x}$.

(b) For each age $x$ and each moment $m$, create the matrices
    $\mathbf{B}_{m}^{(x)}$ and $\mathbf{C}_{m}^{(x)}$ according to (49)
    and (50).

(c) Construct the block-structured matrices $\tilde{\mathbf{B}}_{m}$ and
    $\tilde{\mathbf{C}}_{m}$ using (41) and (43) and assemble the
    matrices $\tilde{\mathbf{R}}_{m}$ as in (12).

(d) Go to 8 .

<!-- -->

8.  Compute the moments of healthy longevity from the transient matrix
    $\tilde{\mathbf{U}}$, the transition matrix $\tilde{\mathbf{P}}$,
    and the reward matrices $\tilde{\mathbf{R}}_{m}$, following
    equations (13) - (15).
