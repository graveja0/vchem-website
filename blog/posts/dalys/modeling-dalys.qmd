...
params <- list(
    ...
    tx_names = c("natural_history"),      # treatment names
...
LE = LEt %>% map(~sum(.x* disc * gen_wcc(params1$omega-1, method = params1$cycle_correction)))
...
params2 <- with(params2,modifyList(params2,list(
    mR = array(as.vector(mR_nh), dim = c(length(tr_names)+ 2,length(tr_names)+ 2,length(tx_names)),
...
result3 <- cbind(LE3 = LE3$natural_history[1], YLD3 = YLD3$natural_history[1], YLL3 = YLL3$natural_history[1],DALY3 = DALY3$natural_history[1]) %>% 
    as.data.frame() %>% 
    mutate_all(~as.numeric(.))  %>% 
    mutate(strategy = "natural_history") %>% 
...
result1 %>% 
    ...
    bind_rows(result3) %>% 
    bind_rows(result4) %>% 
    kable(digits = 3, col.names = c("Approach","Scenario","Life Expectancy","YLDs","YLLs","DALYs","QALY-like DALY")) %>% 
    kable_styling()
...
# Approach 2: Reward Matrix (0.5 correction)
H = matrix(1,nrow=tau, ncol=omega)
h = vec(H) %>% as.matrix()
not_h = 1-h
...
# Approach 3: Markov Trace with Half-Cycle Correction
tr <- 0:omega %>% map_df(~(c(1,0,0,0,0) %*% (t(mP$natural_history) %^% .x) %>% as.data.frame()))
sum( gen_wcc(100,"half-cycle") * (as.matrix(tr) %*% c(1,1,1,0,0)))

# Approach 4: Simple Math 
sum(gen_wcc(100,"half-cycle")*exp(-.1*0:100))

# Approach 5: Partial Reward with Markov Trace and Half-Cycle Correction
tr <- 0:omega %>% map_df(~(c(1,0,0,0,0) %*% (t(mP$natural_history) %^% .x) %>% as.data.frame()))
r_ <- c(0.9,0,0,0,0)
sum( gen_wcc(100,"half-cycle") * ((as.matrix(tr) %*% c(1,1,1,0,0)) %*% r_))

# Approach 6: Partial Reward Matrix  (Should be roughly equivalent to Approach 5)
H = matrix(1,nrow=tau, ncol=omega)
h = vec(H) %>% as.matrix()
not_h = 1-h
...
# Approach 7: Transition Reward for Death based on diff(deaths) in trace
...
# Approach 8: Transition Reward in Markov Chain
