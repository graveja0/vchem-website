...
tr <- trace1$natural_history[-nrow(trace1$natural_history),]
```
...
sum(gen_wcc(params1$omega+1,params1$cycle_correction)*as.matrix(trace1$natural_history) %*% d_)
lt_ %>% filter(age==params1$age0) %>% pull(ex)
```
