...
as.matrix(tr) %*% d_ %>% data.frame() %>% 
    ...
    theme_ipsum() + 
    geom_dl(method = "last.points", aes(label = method, colour = method)) + 
...
## Checkpoint: Percentage of Deaths
...
tr %>% 
  ...
  mutate(OC_ = c(0,diff(DeathOC)),
         CVD_ = c(0,diff(DeathCVD))) %>% 
  mutate(pct_model = CVD_/(CVD_+OC_)) %>% 
  select(age,DeathOC,DeathCVD,CVD_, OC_,pct_model) %>% 
  left_join(
    df_gbd %>% 
    filter(measure_name == "Deaths" & sex_name=="Both" & metric_name=="Percent") %>% 
    inner_join(lut_age_incidence,"age_name") %>% 
    select(age, pct_gbd = val), "age"
  ) %>% 
  data.frame() %>% 
  select(age,pct_model,pct_gbd) %>% 
  gather(method,value,-age) %>% 
  ggplot(aes(x = age, y = value, colour = method)) + geom_point() +  geom_line() +
  geom_dl(method=list("last.points"),aes(label = method)) +
  theme_ipsum() + 
  scale_colour_aaas() + 
...
disc = 
  exp(-params2$r_Delta_t * 0:(params2$omega+1))
...
# # YLL
# 
# Ex <- matrix(0,ncol= 4, nrow = params2$omega)
# colnames(Ex) = rownames(params2$mP$natural_history[[1]])
# rownames(Ex) = params2$ages[-length(params2$ages)]
# Ex[,"trSickDead"] = with(params,(1/r_ann) * (1 - exp(-r_ann * fExR(ages[-1]))))
# 
# YLLt2 = trace2 %>% map(~({
#     apply(.x * Ex  ,1,sum)
#     }))
