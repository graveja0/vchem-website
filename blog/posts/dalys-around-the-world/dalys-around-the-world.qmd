...
Our objective here is to construct a decision model using data and parameters from the 2019 Global Burden of Disease study. 
...
rD <- function(aa, gg = "Both") {
  ...
  pct_cvd <-
      ...
      as.data.table() %>% 
      arrange(age) %>% 
    mutate(pct_cvd = rr_deathCVD * pct_cvd)
  setkey(pct_cvd, age)
...
params1 <- with(params,modifyList(params,list(
    ...
    mR = 
      ages %>% map(~({
        ...
        # mR_nh_p = # Use parametric mortality coefficients rather than life table. 
        #   matrix(c(
        #   -(rH_CVD(.x) + rD_p(.x,params = params)), rH_CVD(.x), rD_p(.x,params = params), 0 ,
        #   0, -(rD_p(.x,params = params)+ rCVD_D_p(.x,params = params)), rD_p(.x,params = params),rCVD_D_p(.x,params = params),
        #   0,0,0,0,
        #   0,0,0,0),
        #   nrow = 4,
        #   ncol = 4,
