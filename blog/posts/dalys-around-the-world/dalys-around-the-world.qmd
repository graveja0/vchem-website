...
- Look into whether we use ages or ages[-1].
...
disc = 
  exp(-params2$r_Delta_t * 0:(params2$omega))
...
# Check Life Expectancy vs. Life Table
d_ <- c(1,1,0,0)
sum(gen_wcc(params2$omega,"half-cycle")*as.matrix(tr) %*% d_)
lt %>% filter(age==params2$age0) %>% pull(ex)
```
